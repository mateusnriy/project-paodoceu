...
import { SearchIcon, PlusIcon, EditIcon, TrashIcon, ChevronLeftIcon, ChevronRightIcon } from 'lucide-react';
...
const AdminProducts: React.FC = () => {
    ...
    return (
      <div className="mt-6 flex items-center justify-center gap-4">
        ...
        <Button
          ...
          variant="outlined" className="px-4 py-2" aria-label="Página anterior"
        >
        ...
        <Button
          ...
          variant="outlined" className="px-4 py-2" aria-label="Próxima página"
        >
  ...
  const renderTableContent = () => {
    ...
    // Tabela normal
    return (
      <table className="min-w-full divide-y divide-gray-200 relative"> {/* Adicionado relative */}
        ...
        <tbody className={`bg-white divide-y divide-gray-200 ${isLoading ? 'opacity-50' : ''}`}> {/* Opacidade durante loading */}
          {products.map((product) => (
            <tr key={product.id} className="hover:bg-gray-50">
               <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4"><div className="text-sm">R$ {formatCurrency(product.preco)}</div></td>
                <td className="px-6 py-4"><div className="text-sm">{product.categoria.nome}</div></td>
                <td className="px-6 py-4"><div className="text-sm">{product.estoque}</div></td>
                ...
                <td className="px-6 py-4 text-right text-sm font-medium">
                  <button onClick={() => handleOpenModal(product)} className="text-accent hover:text-accent/70 mr-3 disabled:opacity-50" disabled={isLoading}>
                    <EditIcon size={18} />
                  </button>
                  <button onClick={() => handleDeleteProduct(product.id)} className="text-error hover:text-error/70 disabled:opacity-50" disabled={isLoading}>
                    <TrashIcon size={18} />
                  </button>
                </td>
            </tr>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => handleOpenModal(null)} color="accent" disabled={isLoading && products.length === 0}>
      ...
      <div className="bg-white rounded-4xl shadow-soft p-6">
        ...
        <div className="overflow-x-auto relative"> {/* Adicionado relative */}
          {renderTableContent()}
        </div>
